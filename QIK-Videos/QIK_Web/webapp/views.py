...
from .forms import TextSearchForm, VideoSearchForm, IndexForm, ExplainForm
...
def video_search(request):
    ...
    if request.method == 'POST' :
        form = VideoSearchForm(request.POST, request.FILES)
        ...
        if form.is_valid():
            ...
            try:
                handleUploadedFile(request.FILES['videoFile'])
            except:
                # Considering the default files provided
                constants.QUERY_VIDEO_PATH = request.POST.get("images", "")
                print("views.py :: video_search :: constants.QUERY_VIDEO_PATH :: " , constants.QUERY_VIDEO_PATH)
            ...
            # Query QIK
            sortedCaptionRanksDict = qik_search.qik_search(constants.QUERY_VIDEO_PATH,fetch_count=k)
            ...
            # Returning the fetched images.
            return render(request, 'webapp/videoResults.html', {'form': form, 'images': sortedCaptionRanksDict, 'imgSrc': constants.QUERY_VIDEO_PATH.replace(constants.QIK_WEBAPP_PATH, constants.QIK_TOMCAT_URL), 'rankingFunc':ranking_func, 'kValue':k, 'searchModel':search_model})
        ...
        if 'query_image_path' in request.GET and 'ranking_function' in request.GET:
            ...
            subprocess.call(shlex.split("cp " + query_image_path + " " + constants.QUERY_VIDEO_PATH))
            ...
            # Query QIK
            sortedCaptionRanksDict = qik_search.qik_search(query_image_path, fetch_count=int(k))
            ...
            # Returning the fetched images.
            return render(request, 'webapp/videoResults.html', {'form': form, 'images': sortedCaptionRanksDict, 'imgSrc': query_image_path.replace(constants.QIK_WEBAPP_PATH, constants.TOMCAT_IP_ADDR + constants.IMAGE_DATA_DIR), 'rankingFunc':ranking_func, 'kValue':k, 'searchModel':search_model})

        else:
            caption_generator.init()

    form = VideoSearchForm();
    return render(request, 'webapp/videoSearch.html', {'form':form})

def handleUploadedFile(f) :
    print("Uploading file upload")

    with open(constants.QUERY_VIDEO_PATH, 'wb+') as dest :
    ...
    if request.method == 'POST' :
        ...
        if form.is_valid():
            ...
            try:
                ...
                print("views.py :: add_index :: Exception encountered while uploading the file.")

            # Indexing the data.
            index_qik.add_index(constants.QUERY_VIDEO_PATH)
...
def explain_query(request):
    if request.method == 'POST' :
        ...
        if query_image_path is None:
            ...
            query_image_path = constants.QUERY_VIDEO_PATH
        ...
        qik_search.qik_search(constants.QUERY_VIDEO_PATH, ranking_func, obj_det_enabled=obj_det_enabled,
