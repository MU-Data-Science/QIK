...
# Configurations
PERCENTAGE = 1  # Percentage for re-ranking
TARGET_DIR = "MSR_VTT_Data_Dataset/TrainValVideo"
QUERY_DIR = "MSR_VTT_Data_Dataset/TestVideo"
FEATURE_DIR = "/mydata/MSR_VTT_Feature_Data"
GROUND_TRUTH = "MSRVTT_All_Data_Categories_Ground_Truth.pkl"
RESULTS_FILE_PREFIX = "dns_msr_vtt_q_testset_d_trainvalset_results"
...
if __name__ == '__main__':
    ...
    if Path(RESULTS_FILE_PREFIX + ".txt").exists():
        os.remove(RESULTS_FILE_PREFIX + ".txt")
    ...
    for query in query_lst:
        print("eval_dns.py :: processing query :: ", query)

        try:
            # Check if video features are present
            if not os.path.isfile(FEATURE_DIR + "/" + query + ".pickle"):
                # Extracting query features
                print("eval_dns.py :: Extracting query features for the query :: ", query)
            ...
            with open(FEATURE_DIR + "/" + query + ".pickle", 'rb') as f:
                ...
                for targ in target_lst:
                    ...
                    try:
                        ...
                        if not os.path.isfile(FEATURE_DIR + "/" + targ + ".pickle"):
                            ...
                            print("eval_dns.py :: Extracting query features for the targ :: ", targ)
                        ...
                        print("eval_dns.py :: Exception occurred processing targ :: ", targ)
                ...
                print("eval_dns.py :: Re-ranking the results for query :: ", query)
                ...
                if len(percent_ranked_selector_scores_lst):
                    for idx in percent_ranked_selector_scores_lst:
                        ...
                        # Check if video features are present
                        if not os.path.isfile(FEATURE_DIR + "/" + targ + ".pickle"):
                            print("eval_dns.py :: Pre-constructed features not available while ranking. Skipping for raking for targ :: ", targ)
                            continue
            ...
            print("eval_dns.py :: Exception occurred processing query :: ", query)
            print("eval_dns.py :: Exception :: " + str(e))
            traceback.print_exc()
    ...
    print("eval_dns.py :: mAP@2: ", get_mAP(relevance_2_lst))
    print("eval_dns.py :: mAP@4: ", get_mAP(relevance_4_lst))
    print("eval_dns.py :: mAP@8: ", get_mAP(relevance_8_lst))
    print("eval_dns.py :: mAP@16: ", get_mAP(relevance_16_lst))
